...
# Hide Streamlit Community Cloud's own top-right toolbar (GitHub/star/fork/share
# icons). These are injected by the hosting platform itself, outside the app's
# own DOM, so they cannot be recolored via CSS from within the app — the only
# reliable way to deal with them is to hide the whole toolbar via the official
# `embed=true` URL flag. We do this once, automatically, via a tiny JS redirect.
if "embed" not in st.query_params:
    st.components.v1.html(
        ...
        (function() {
            ...
            } catch (e) { /* cross-origin or blocked — fail silently */ }
        })();
        </script>
        """,
        height=0,
