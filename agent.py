...
def render_home():
    ...
    with c2:
        if st.button("📷 Upload Photo", use_container_width=True):
            st.info("Photo recognition isn't wired up in this demo yet — please type your ingredients instead.")
    with c3:
    ...
    if st.session_state.data["ingredients"]:
        ...
        names = ", ".join(i["name"] for i in st.session_state.data["ingredients"])
        st.markdown(f'<span class="chip">{names}</span>', unsafe_allow_html=True)
...
def render_ingredients():
    ...
    with st.form("add_ingredient", clear_on_submit=True):
        ...
        qty = c2.text_input("Quantity", placeholder="e.g. 500g")
        submitted = c3.form_submit_button("➕ Add", use_container_width=True)
        if submitted and name.strip():
            st.session_state.data["ingredients"].append({"name": name.strip(), "qty": qty.strip() or "—"})
            save_data()
            st.rerun()
    ...
    for i, ing in enumerate(ingredients):
        with cols[i % 4]:
            st.markdown(
                ...
                <b>{ing['name']}</b><br><span style="opacity:0.7; font-size:12.5px;">{ing['qty']}</span>
                </div>""",
                unsafe_allow_html=True,
            )
...
def render_shopping_list():
    ...
    with st.form("add_item", clear_on_submit=True):
        ...
        qty = c2.text_input("Quantity", placeholder="e.g. 3-4 pieces")
        submitted = c3.form_submit_button("➕ Add", use_container_width=True)
        if submitted and name.strip():
            items.append({"name": name.strip(), "qty": qty.strip() or "—", "checked": False})
            save_data()
            st.rerun()
