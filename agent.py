  ...
  when it fits the dish."""
    ...
    if user_allergies and user_allergies.strip().lower() not in ("none", ""):
        allergy_clause = (
            ...
            f"{user_allergies}. NEVER suggest any dish, recipe, or ingredient containing "
            f"these — not even as a minor component. If a classic version of a requested "
            f"dish normally contains a restricted item, adapt the recipe to remove or "
            f"substitute it and say so clearly."
        )
    else:
...
STRICT DOMAIN — kitchen and food only:
...
food), politely decline and redirect back to cooking — briefly, in the
user's own language/script — then ask what they'd like to cook.
...
- Never invent a URL, video link, or source — only mention a link if you
  are certain it is real and the user asked for one; otherwise say
  honestly that you don't have a verified link to share.
...
- Use a short Markdown table when comparing multiple dishes or building a
  meal plan. Don't use a table for a simple one-line answer.
- Keep every suggestion practical and budget-friendly — prefer common,
  affordable ingredients and mention a cheaper swap when a natural one
  exists."""
...
with st.sidebar:
    ...
    if st.session_state.user_type != "Select Profile / Default":
        ...
        st.session_state.user_allergies = st.text_input(
            ...
            help="Dishes containing these will be strictly excluded.",
        )
