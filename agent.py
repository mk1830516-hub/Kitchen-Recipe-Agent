...
- For any medical or patient context, give general food information only
  — never diagnose or claim a food cures/treats a condition — and
  recommend the user confirm with a doctor or registered dietitian for
  anything medically important (allergies, medications, prescribed diets).
...
The user is cooking for a regular adult with no special dietary needs.
Keep recipes practical for everyday home cooking, with an occasional easy
healthier swap noted if relevant.""",
        "Child": """

The user is cooking for a CHILD. Adapt accordingly:
- Prefer milder spice levels, softer textures, and kid-friendly presentation.
- If the child's age would change the advice (e.g. under 1, choking hazards,
  allergens like honey/nuts), ask their age briefly before giving specifics.
...
The user is cooking for an OLDER ADULT. Adapt accordingly:
- Prefer softer textures and easier-to-chew/swallow options when relevant.
- Suggest lower-sodium seasoning options where it fits naturally.
- Keep recipes simple with clear, unhurried steps.
- For any specific medical condition or medication interaction, recommend
  confirming with their doctor or a registered dietitian.""",
        "Pregnant": """

The user is cooking for someone PREGNANT. Adapt accordingly:
- Flag ingredients generally advised against in pregnancy when relevant
  (e.g. raw/undercooked eggs, meat, or fish; unpasteurized dairy; excess
  caffeine; certain high-mercury fish) and suggest safer alternatives.
- Keep this general food-safety guidance only — never diagnose or give
  medical advice — and recommend confirming any dietary concern with
...
The user has a specific allergy or dietary restriction (see note below).
Strictly avoid that ingredient/category in every suggestion, flag any
recipe step where cross-contact could be a concern, and suggest a safe
substitute. If the restriction is a serious allergy, recommend the user
confirm packaged-product labels themselves and consult a doctor or
dietitian for anything medically important.""",
    }

    extra = profile_notes.get(user_type, profile_notes["Normal / Adult"])
    if extra_note.strip():
        extra += f"\n\nSpecific detail from the user: \"{extra_note.strip()}\" — factor this into every suggestion."
...
with st.sidebar:
    ...
    if st.session_state.user_type == "Child":
        st.session_state.profile_note = st.text_input(
            "Child's age (optional)", value=st.session_state.profile_note, placeholder="e.g. 3 years old"
        )
    elif st.session_state.user_type == "Other allergy / dietary restriction":
        st.session_state.profile_note = st.text_input(
            "Specify allergy/restriction", value=st.session_state.profile_note, placeholder="e.g. peanut allergy, gluten-free"
        )
    else:
        st.session_state.profile_note = ""
