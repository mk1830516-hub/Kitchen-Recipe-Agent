...
- Only suggest foods that are generally recognized as safe for a {pet_label.lower()}.
  If dog-safe and cat-safe advice genuinely differs, say so clearly.
- ALWAYS explicitly warn against and never include known toxic foods for
  pets: chocolate, grapes/raisins, onion, garlic, chives, xylitol
  (artificial sweetener), macadamia nuts, alcohol, caffeine, raw bread
  dough, and cooked bones (choking/splintering risk).
...
- Add a note recommending the user confirm with a veterinarian before
  introducing a new food, especially for puppies/kittens or pets with
  health conditions."""
    else:  # Adult
...
st.markdown(
    ...
    /* Force light on Streamlit's own chrome regardless of system dark mode
       or whether config.toml has been redeployed yet */
...
with st.sidebar:
    ...
    if current_type == "Infant / Baby (Under 1 year)":
        ...
        st.session_state.user_allergies = st.text_input(
            ...
            help="Leave blank or type 'none' if not applicable.",
        )
    elif current_type == "Pet (Dog / Cat)":
        ...
        st.session_state.user_allergies = st.text_input(
            ...
            help="Foods this pet should avoid, if known.",
        )
        ...
        st.session_state.user_allergies = st.text_input(
            ...
            help="Dishes containing these will be strictly excluded — any language works.",
        )
...
def render_home():
    ...
    elif current_profile == "Pet (Dog / Cat)":
        profile_badge = (
